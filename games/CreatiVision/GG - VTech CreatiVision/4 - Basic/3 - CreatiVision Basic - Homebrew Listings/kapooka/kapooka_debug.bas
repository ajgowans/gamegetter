...
REM 370  PRINT L + (B*W) + A, PEEK(L+(B*W)+A)
372  NEXT A
374  NEXT B
...
605  A = Z+2
REM 608  PRINT L+A,PEEK(L+A-1),PEEK(L+A)
615  GOSUB 5000
620  PLOT X,Y,16
625  PLOT X-1,Y,0
628  POKE L+A, 16
...
705  A = Z-2
REM 708  PRINT L+A,PEEK(L+A-1),PEEK(L+A)
715  GOSUB 5000
720  PLOT X,Y,16
725  PLOT X+1,Y,0
728  POKE L+A, 16
...
805  A = Z+(W*2)
REM 808  PRINT L+A,PEEK(L+A+W),PEEK(L+A)
815  GOSUB 5000
820  PLOT X,Y,16
825  PLOT X,Y-1,0
828  POKE L+A, 16
...
905  A = Z-(W*2)
REM 908  PRINT L+A,PEEK(L+A-W),PEEK(L+A)
915  GOSUB 5000
920  PLOT X,Y,16
925  PLOT X,Y+1,0
928  POKE L+A, 16
